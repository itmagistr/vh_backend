...
<!--        <div class="bgz-main"></div>-->
        ...
        <div class="service">
            <div v-for="c in category" class="card-tooth"
                 :key="c.code"
                 :class="[{active: catSel === c.code}]" @click="chgCat(c)">
              <div>
                <img :src="c.img"/>
                <div>{{c.title}}</div>
              </div>
...
export default {
    ...
    components: { 
      info, listView, modalDocCard, modalDoctorList
    },
    layout: 'def',
    ...
    methods: {
      ...
      async preBookingReq() {
        await fetch(`${this.$store.state.apihost}${this.$i18n.locale}/vhapi/category/PRE_BOOKING/`).
      ...
      async categoryList() {
        await fetch(`${this.$store.state.apihost}${this.$i18n.locale}/vhapi/category/`).
        then(response => response.json()).
