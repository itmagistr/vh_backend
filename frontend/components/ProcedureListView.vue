<template>
  <div class="listView">
    <template v-for="c in results">
      <div :key="c.uid" :class="['product', {spec: c.code.length > 5}, { active: c.uid === prselect}]"
          ...
          <div class="sr-end">
            <div v-if="infoButton" class="pr-info" @click="detail(c.uid)"><i class="fas fa-info-circle"></i></div>
      ...
      <info v-if="prselect === c.uid && resize && infoBlock" :key="'block-' + c.uid" 
        :showDoc="showDoc" class="hm-block" :info="prselect"
...
export default {
    ...
    methods: {
      ...
      medProcListByDoc() {
        ...
        fetch(`${this.$store.state.apihost}${this.locale}/vhapi/doctor/${this.docSel}/medprocs/`).
        then(response => response.json()).
