...
class MedProcAdmin(TranslationAdmin):
	...
	form = MedProcAdminForm
	actions = ["export_medprocs", ]
	def export_medprocs(self, request, queryset):
		f = StringIO()
		writer = csv.writer(f, delimiter=str(';'))
		writer.writerow(u'\ufeff')
		writer.writerow([u"medproc_id", u"medproc_uid", u"medproc_code", u"title_check_ru", u"title_check_en", u"title_ru", u"title_en",u"price", u"price_old", u"duration",
		...
		for s in MedProc.objects.all().order_by('code'):
			...
			writer.writerow([s.id, s.uid, s.code, s.title_check_ru, s.title_check_en, s.title_ru, s.title_en, int(s.price), int(s.price_old), s.duration, 
